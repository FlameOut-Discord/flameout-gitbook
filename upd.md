...
1. Встречайте — промокоды в FlameOut! Их можно получить при раздачах, конкурсах, розыгрышах, за предложение идей по улучшению бота, за поиск багов и много чего ещё.\
   Ловите промокод: **`4Iy4lcc6`**, действует до 30 августа. Активировать: **`f!promocode`** 🎫
2. 📑 Создали пользовательское соглашение. Продолжая пользоваться ботом и остальными нашими сервисами, Вы автоматически принимаете пользовательское соглашение

{% content-ref url="documents/user-agreement.md" %}
[user-agreement.md](documents/user-agreement.md)
{% endcontent-ref %}

1. Добавили больше алиасов для вашего удобства :)
2.  Теперь можно установить свой регион в FlameOut (**`f!setregion <страна> <город>`**) — он требуется для доступа к некоторым командам.\
...
3. Починили команды **`f!hiddenmode`**, **`f!bird`**, **`f!8ball`** теперь снова работают
4. Пофикшена команда **`f!color`**. Теперь бот не молчит, если пользователь ошибся или у API проблемы.
5. Дуэли снова доступны! Команда **`f!duel`** пофикшена
6. Покупка бамповой кирки в **`f!shop`** пофикшена.
7. Теперь бот нормально реагирует на текст содержащий более 4028 символов в команде **`f!lyrics`**

<figure><img src=".gitbook/assets/image (3).png" alt=""><figcaption><p>*трек взят в качестве примера.</p></figcaption></figure>

1. Откатились до более старой версии пакета discord.js, теперь кнопки и менюшки снова работают, а бот — не падает :)
2. Топы по монетам и биткоинам пофикшены.
3. Теперь шар (**`f!8ball`**) не отвечает на сообщения без вопроса.
...
5.  Теперь можно использовать **`f!weather`** без указания города, если в Вашем профиле FlameOut установлен регион.\
    &#x20;

    <figure><img src=".gitbook/assets/image (6).png" alt=""><figcaption></figcaption></figure>
6.  В команде **`f!lyrics`** теперь стоит плашка, что текст трека будет недоступен до официальной публикации трека исполнителем\
    &#x20;

    <figure><img src=".gitbook/assets/image (25).png" alt=""><figcaption></figcaption></figure>
7. Теперь дворник зарабатывает 1500 монет за рабочую смену.
8. Обновили команду **`f!language`**, изменили стиль и добавили больше информации о текущих доступных языках в FlameOut
9. Теперь количество получаемого опыта в **`f!school`** зависит от региона который установил пользователь.
...
1.  Обновили обложку в группе ВКонтакте\
